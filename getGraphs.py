...
class GetGraphs:
	...
	def graphs(self,directory, location, dirList):
		graphs = []
		dirList.sort()
		print(dirList)
		for j in range(len(dirList) - 1):
			with open(directory + str(location) + "/" + dirList[j], "r") as file:
				gp = json.load(file)
			graphs.append(gp)

		return graphs

	def getTestGraph(self, directory, location, filename):
		with open(directory + str(location) + "/goodset/" + filename, "r") as file:
			gp = json.load(file)
