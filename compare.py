...
for i in range(len(questionList)):
	...
	dirList = os.listdir("data3/" + questionList[i])  # dir is your directory path
	x = GetGraphs(len(dirList) - 1)
	trainData = x.graphs('data3/' , questionList[i], dirList)
	dirList = os.listdir("data3/" + questionList[i] + "/goodset")
	# print(dirList)
	for filename in dirList:
		test = x.getTestGraph('data3/', questionList[i], filename)
	...
	for j in range(len(dirList), len(corpusB)):
		...
		for k in range(len(dirList)):
			...
			totalavg = sumB + sumE + sumEC + sumBC + sumED
		...
		bavg = numpy.sum(basicVector[:25]) / min(len(basicVector), 25)
		ecavg = numpy.sum(controlContextECVector[:25]) / min(len(controlContextECVector), 25)
		expavg = numpy.sum(exprVector[:25]) / min(len(exprVector), 25)
		ccavg = numpy.sum(controlContextBCVector[:25]) / min(len(controlContextBCVector), 25)
		edavg = numpy.sum(exprDependVector[:25]) / min(len(exprDependVector), 25)



		basicAll.append(bavg)
		controlContextECAll.append(ecavg)
		exprAll.append(expavg)
		controlContextBCAll.append(ccavg)
		exprDependAll.append(edavg)

rows = []
with open('dataset_new2/worksheet.csv', 'rb') as f:
	reader = csv.reader(f)
	for row in reader:
		rows.append(row)
...
with open('dataset_new2/worksheet.csv', 'wb') as f:
	writer = csv.writer(f)
