...
for i in range(len(questionList)):
	...
	dirList = os.listdir(dataPath + questionList[i])  # dir is your directory path
	x = GetGraphs(len(dirList) - 1)
	trainData = x.graphs(dataPath, questionList[i], dirList)
	dirList = os.listdir(dataPath + questionList[i] + "/goodset")
	for filename in dirList:
		test = x.getTestGraph(dataPath, questionList[i], filename)
...
rows = []
with open(datasetPath + 'worksheet.csv', 'rb') as f:
	reader = csv.reader(f)
	for row in reader:
		rows.append(row)
...
with open(datasetPath + 'worksheet.csv', 'wb') as f:
	writer = csv.writer(f)
